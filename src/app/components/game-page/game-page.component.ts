...
export class GamePageComponent implements OnInit {
	...
	public currantPointsArray = [	// Массив с попаданиями с трёх полей каждоко игрока
		[
			{ dart: 0 },
			{ dart: 0 },
			{ dart: 0 },
			...
			for (let j = 0; j < this.userService.usersGameArray.length; j++) {
				if (ind == j) {
					...
					if (x > 1) {
						...
						this.winnerUser = this.userService.usersGameArray[j].name;
						break;
					}
					// else alert(`${this.userService.usersGameArray[j].name} набрал(а): ${this.userService.currantTotalPointsArray[j].totalPoint} очков - не сможет выйти по удвоению.`);
						...
						this.impossobility.user = this.userService.usersGameArray[j].name;
						this.impossobility.point = this.userService.currantTotalPointsArray[j].totalPoint;
						setTimeout(() => {
							this.impossobility.status = false;
						}, 5000);
					};
